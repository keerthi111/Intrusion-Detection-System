...
#define TOTAL_OBJECT              24           //must be an even number
...
void Decision_Rule( vector <vector <float> > & input,vector <pair <int , double > > &best_attr, vector <int> &final_reduct){
	...
	outfile4.open("rules_class1");              //Declare a file to store rule in understandable formate
	outfile5.open("rules_class1_binary");       //Declare a file to store rule in binary formate
	ofstream outfile9;                          //Declare variable for output file
	outfile9.open("reduct_attribute_index_class1");    //Declare a file to store index of attribute
